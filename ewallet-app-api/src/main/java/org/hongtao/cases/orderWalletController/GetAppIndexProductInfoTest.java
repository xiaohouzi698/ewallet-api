...
import static org.hongtao.config.TestConfig.getAppIndexProductInfoUrl;

public class GetAppIndexProductInfoTest {

    private static Logger logger = Logger.getLogger(String.valueOf(GetAppIndexProductInfoTest.class));

    @BeforeTest(groups = "appRegister", description = "测试准备工作，获取 HttpClient 对象")
    public void beforeTest() {
        getAppIndexProductInfoUrl = ConfigFile.getUrl(InterfaceName.GETAPPINDEXPRODUCTINFO);
        appUserLoginUrl = ConfigFile.getUrl(InterfaceName.APPUSERLOGIN);
    }


    @Test(description = "getAppIndexProductInfoTest")
    public void getAppIndexProductInfoTest() throws IOException, URISyntaxException, BadPaddingException, NoSuchAlgorithmException, IllegalBlockSizeException, NoSuchPaddingException, InvalidKeyException, InvalidKeySpecException {
    ...
    private JSONObject getJsonResult( String token ) throws IOException, URISyntaxException {
        ...
        URIBuilder uriBuilder = new URIBuilder(getAppIndexProductInfoUrl);
